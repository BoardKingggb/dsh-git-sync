...
export async function repoExists(token: string, owner: string, name: string): Promise<boolean> {
 ...
 */
export async function tokenFor(config: Config, env: NodeJS.ProcessEnv = process.env): Promise<string | undefined> {
...
function storedGitHubToken(): Promise<string | undefined> {
...
export async function ensureRepoExists(
  ...
  if (parsed === undefined) return 'skipped'
  const token = await tokenFor(config, env)
  if (token === undefined) return 'skipped'
